...
def momentum_strategy(start_date, end_date, lookback, portfolio_weight):
    ...
    for i in range(1, len(months)):
        ...
        if lookback == "3-month":
            adjusted_start_date = start_month - pd.DateOffset(days=91)
            adjusted_end_date = start_month
        elif lookback == "6-month":
            adjusted_start_date = start_month - pd.DateOffset(days=182)
            adjusted_end_date = start_month
        elif lookback == "9-month":
            adjusted_start_date = start_month - pd.DateOffset(days=273)
            adjusted_end_date = start_month
        elif lookback == "11-month-skip-1-month":
            adjusted_start_date = start_month - pd.DateOffset(days=364)
            adjusted_end_date = start_month - pd.DateOffset(days=28)
